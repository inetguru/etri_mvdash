...
#ifndef MVDASH_HELPER_H
#define MVDASH_HELPER_H
...
public:
   ...
   *
   * \param address the value of the application attribute to set
   * \param bUseQuic 
   */
  mvdashServerHelper (Address address, uint16_t bUseQuic);
  /**
   * Create etri_mvdashClientHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   ...
   */
  mvdashServerHelper (Ipv4Address ip, uint16_t port, uint16_t bUseQuic);
...
public:
   ...
   * \param bUseQuic 
   */
  mvdashClientHelper (Address serverAddress, uint16_t bUseQuic);
  /**
   * Create etri_mvdashClientHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   ...
   * \param port The port number of the remote tcp stream server
   */
  mvdashClientHelper (Ipv4Address serverIP, uint16_t serverPort, uint16_t bUseQuic);
   ...
   */
  ApplicationContainer Install (NodeContainer c, std::string algo) const;
...
private:
   ...
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node, uint16_t clientId) const;
  ObjectFactory m_factory; //!< Object factory.
};

} // namespace ns3

#endif /* MVDASH_HELPER_H */