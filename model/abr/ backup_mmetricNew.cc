  ...
  if (tIndexReq > 0)
    {
      ...
      std::vector<double> vmaf_defauld (m_nViewpoints);
      for (int vp = 0; vp < m_nViewpoints; vp++)
        {
          ...
          if (prev_mainView != curViewpoint && vp == curViewpoint)
            vmaf_last[vp] = m_videoData[prev_mainView]
                                .vmaf[m_downData.qualityIndex[idLast][prev_mainView]][segmentLast];
          else
            {
              vmaf_last[vp] =
                  m_videoData[vp].vmaf[m_downData.qualityIndex[idLast][vp]][segmentLast];
            }
        }
      ...
      // NS_LOG_INFO ("------------------------------------------------!"
      //              << CHUNK_COMBO_OPTIONS.size ());

      for (int sgmt i = 0; i < future_chunk_length; i++)
        {
          int sgmt_idx = i + tIndexReq;
          double curr_buffer = start_buffer;
          double curr_rebuffer_time = 0;
          double bitrate_sum = 0;
          double smooth_diff = 0;
          std::vector<double> vmaf_last (m_nViewpoints) = vmaf_defauld;
          std::vector<std::vector<int>> CHUNK_COMBO_VP =
              combinatorial_Viewpoint (curViewpoint, a_dim, m_nViewpoints, sgmt_idx);
          double VP_reward = 0;
          for (auto &combo : CHUNK_COMBO_VP)
            {

              double VP_curr_buffer = curr_buffer;
                  ...
                  if (pos == curViewpoint)
                    {
                      ...
                      smooth_diff += 1 * std::pow ((vmaf_score - vmaf_last[pos]), 2);
                    }
                  else
                    {
                      rate = Qsubtarget - vmaf_score;
                      bitrate_sum += 0.001 * std::pow (rate, 2);
                      smooth_diff += 0.001 * std::pow ((vmaf_score - vmaf_last[pos]), 2);
                    }
                }
              ...
              VP_reward = std::pow (bitrate_sum, 2) + std::pow (curr_rebuffer_time, 2) +
