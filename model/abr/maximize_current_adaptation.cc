  ...
  if (tIndexReq > 0)
    {
      ...
      //Estimate BW
      double bw_prev = (double) dataSize * m_videoData[0].segmentDuration / tDelay;
      ...
      if (prevViewpoint != curViewpoint)
        {
          buffer_now = m_bufferData[prevViewpoint].bufferLevelNew.back () -
                       (timeNow - m_downData.time.back ().downloadEnd);
        }
      ...
      int64_t tAvailable = buffer_now - lowerBound;
      ...
      */
      ...
      int64_t dataAllowed =
          (int64_t) bw_prev * tAvailable / m_videoData[0].segmentDuration - other_dataSizeToSend;
...
int64_t
maximizeCurrentAdaptation::dataSize_last (int32_t idLast, std::vector<int32_t> *pIndexes,
                                          int prevViewpoint)
{
  int64_t dataSize = 0;
...
int64_t
maximizeCurrentAdaptation::dataSize_predict (bool isGroup, int segmentID,
